...
function createContactModal() {
  ...
  m.innerHTML = `<div class="bg-zinc-900 border border-zinc-700/80 rounded-3xl w-full max-w-sm p-6">
...
function rebuildCategorySelect() {
  ...
  sel.innerHTML = "";
  ["Payment","Gift","Work","Other",...allCategories.map(c=>c.name)].forEach(name => {
...
function disconnectWallet() {
  ...
  $("walletStatus").innerHTML = ""; $("balanceDisplay").textContent = "";
...
$("chooseContactBtn").onclick = () => {
  ...
  $("contactSearchModal").value=""; renderContactModalList(""); $("contactPickModal").classList.remove("hidden");
};
...
$("miniSaveEventBtn").onclick=async()=>{
  if(!account){showToast("Connect wallet first!","error");return;}
  if(!selectedMiniDay){showToast("Select a day","error");return;}
  const title=$("miniEventTitle").value.trim();
  if(!title){showToast("Enter a title","error");return;}
  const type=$("miniEventType").value, color=TYPE_COLORS[type]||"#6366f1";
  ...
  showMiniDayDetail(selectedMiniDay.d,selectedMiniDay.y,selectedMiniDay.m);
