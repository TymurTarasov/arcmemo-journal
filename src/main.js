...
function renderContactModalList(q = "") {
  ...
  if (!filtered.length) {
    list.innerHTML = '<p class="text-zinc-600 text-xs text-center py-6">No contacts found</p>';
    return;
  }
  list.innerHTML = filtered.map(c =>
    ...
    '<div class="w-9 h-9 rounded-full bg-emerald-500/15 text-emerald-400 flex items-center justify-center font-semibold text-xs shrink-0">' +
    c.name.slice(0,2).toUpperCase() + '</div>' +
    '<div class="flex-1 min-w-0">' +
    '<div class="text-sm font-medium">' + c.name + '</div>' +
    '<div class="font-mono text-xs text-zinc-500 truncate">' + c.address + '</div>' +
    '</div><span class="text-emerald-500 text-xs">→</span></div>'
  ).join("");
}
...
function renderSavedCategories() {
  if (!allCategories.length) {
    savedCategoriesList.innerHTML = '<span class="text-xs text-zinc-600">No custom categories yet — add one below</span>';
    return;
  }
  savedCategoriesList.innerHTML = allCategories.map(c =>
    ...
    '<button onclick="renameCategory(' + c.id + ',\'' + c.name + '\')" class="text-zinc-600 hover:text-blue-400 text-xs ml-1.5 transition" title="Rename">✎</button>' +
...
window.addEventListener("load", async () => {
  ...
  try {
    ...
    if (accounts.length > 0) {
      ...
      updateWalletUI();
      await switchToArc();
      await Promise.all([loadBalance(), loadHistory(), loadContacts(), loadCategories(), loadScheduled()]);
      updateDepositUI();
    }
...
function disconnectWallet() {
  ...
  renderHistory([]); renderContacts([]); updateStats();
...
sendBtn.onclick = async () => {
  ...
  try {
    ...
    await supabase.from("transactions").insert({
      wallet: account.toLowerCase(), recipient: to,
      amount: parseFloat(amount), memo, category, txhash: hash,
    });
...
addContactBtn.onclick = async () => {
  ...
  const address = contactAddressEl.value.trim();
...
saveScheduleBtn.onclick = async () => {
  ...
  const amount    = schedAmount.value;
  ...
  const { error } = await supabase.from("scheduled_payments").insert({
    wallet: account.toLowerCase(), recipient,
    amount: parseFloat(amount), memo,
    category: "Payment",
    scheduled_at: new Date(dateTime).toISOString(),
    repeat_type: repeat, status: "pending",
  });
  if (error) { showToast("Error: " + error.message, "error"); return; }
...
async function loadScheduled() {
  ...
  items.forEach(p => scheduleNotification(p));
}
...
function renderScheduled(items) {
  ...
  const repeatLabels = { once:"One time", daily:"Daily", weekly:"Weekly", monthly:"Monthly" };
  scheduledList.innerHTML = items.map(p => {
    ...
    return (
      ...
      (isDue ? 'border-violet-500/40 bg-violet-500/5' : 'border-zinc-700/50 bg-zinc-800/30') + '">' +
      '<div class="w-9 h-9 rounded-full flex items-center justify-center text-base shrink-0 ' +
      (isDue ? 'bg-violet-500/20 text-violet-400' : 'bg-zinc-700/60 text-zinc-400') + '">⏰</div>' +
      '<div class="flex-1 min-w-0">' +
      '<div class="flex items-start justify-between gap-2">' +
      '<div>' +
      '<div class="text-sm font-medium">' + (name || p.recipient.slice(0,8)+"..."+p.recipient.slice(-4)) + '</div>' +
      ...
      '</div>' +
      '<div class="text-right shrink-0">' +
      '<div class="text-sm font-semibold text-violet-400">' + p.amount + ' USDC</div>' +
      '<div class="text-xs text-zinc-600">' + repeatLabels[p.repeat_type] + '</div>' +
      '</div></div>' +
      (p.memo ? '<div class="text-xs text-zinc-600 mt-1.5">💬 ' + p.memo + '</div>' : '') +
      '<div class="flex gap-2 mt-3">' +
      (isDue
        ? '<button onclick="executeScheduled(' + p.id + ',\'' + p.recipient + '\',' + p.amount + ',\'' + (p.memo||"") + '\',\'' + p.category + '\',\'' + p.repeat_type + '\')" ' +
          'class="flex-1 py-2 bg-violet-500/20 hover:bg-violet-500/40 border border-violet-500/30 text-violet-300 text-xs font-medium rounded-xl transition">⚡ Send Now — confirm in MetaMask</button>'
        : '<div class="flex-1 py-2 text-center text-zinc-600 text-xs border border-zinc-800 rounded-xl">Waiting for scheduled time...</div>'
      ) +
...
window.executeScheduled = async (id, recipient, amount, memo, category, repeatType) => {
  if (!account) { showToast("Connect wallet first!", "error"); return; }
  showToast("Opening MetaMask to confirm...", "info");
  try {
    const wc = createWalletClient({ chain: ARC_TESTNET, transport: custom(window.ethereum) });
...
function renderCalendar() {
  ...
  for (let day=1; day<=daysInMonth; day++) {
    const h = txByDay[day];
    html += '<div onclick="showDayDetails(' + day + ',' + year + ',' + month + ')" class="py-1.5 rounded-xl text-center cursor-pointer text-xs ' +
      (h ? "bg-emerald-500/20 text-emerald-400 font-semibold" : "hover:bg-zinc-800 text-zinc-500") + '">' + day + '</div>';
  }
...
window.showDayDetails = (day, year, month) => {
  ...
  dayTransactionsEl.innerHTML = txs.map(tx => {
    const name = getContactName(tx.recipient);
    return (
      '<div class="flex justify-between items-center p-2.5 bg-zinc-800/60 rounded-xl text-xs">' +
      '<span class="text-zinc-300">' + (name || tx.recipient.slice(0,8)+"...") + '</span>' +
