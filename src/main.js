...
function createContactModal() {
  ...
  m.innerHTML = `
    <div class="bg-zinc-900 border border-zinc-700 rounded-3xl w-full max-w-sm p-6">
      <div class="flex items-center justify-between mb-4">
        <h3 class="text-sm font-semibold">Choose a contact</h3>
        <button id="closeContactPickModal" class="text-zinc-500 hover:text-white text-lg leading-none">✕</button>
      </div>
...
function renderContactModalList(q = "") {
  ...
  list.innerHTML = f.map(c =>
    '<div onclick="pickContactItem(\'' + c.address + '\',\'' + c.name + '\')" ' +
...
$("category").addEventListener("change", () => {
  $("category").style.color = getCategoryColor($("category").value);
});
...
function renderSavedCategories() {
  ...
  if (!allCategories.length) {
    el.innerHTML = '<span class="text-xs t3">No custom categories yet</span>';
    return;
  }
  el.innerHTML = allCategories.map(c =>
    '<div class="flex items-center gap-1.5 rounded-xl px-2.5 py-1.5 border group" ' +
    'style="background:' + (c.color||"#6366f1") + '22;border-color:' + (c.color||"#6366f1") + '55">' +
...
window.selectCategory = name => {
  $("category").value = name;
  $("category").style.color = getCategoryColor(name);
...
$("addCategoryBtn").onclick = async () => {
  ...
  if (allCategories.find(c => c.name.toLowerCase() === name.toLowerCase())) {
    showToast("Already exists", "info"); return;
  }
...
function updateWalletUI() {
  ...
  btn.className = "px-5 py-2.5 bg-zinc-800 hover:bg-red-900/30 text-zinc-400 hover:text-red-400 text-sm font-medium rounded-2xl transition border border-zinc-700";
...
function disconnectWallet() {
  ...
  btn.className = "px-5 py-2.5 bg-white text-black text-sm font-medium rounded-2xl hover:bg-zinc-100 transition";
  btn.onclick = async () => {
    try {
      const accounts = await window.ethereum.request({ method: "eth_requestAccounts" });
...
$("exportBtn").onclick = () => {
  ...
  const rows = [
    ...
    ...allHistory.map(tx => [
      ...
      getContactName(tx.recipient)||"", tx.recipient,
      tx.amount, tx.memo||"", tx.category, tx.txhash||""
    ])
  ];
  const a = document.createElement("a");
  a.href = URL.createObjectURL(new Blob([rows.map(r=>r.join(",")).join("\n")], { type:"text/csv" }));
  a.download = "memojournal.csv"; a.click();
...
function renderContacts(contacts) {
  ...
  el.innerHTML = contacts.map(c =>
    ...
    '<button onclick="deleteContact(' + c.id + ')" class="text-xs px-2 py-1 hover:bg-red-500/20 t3 hover:text-red-400 rounded-lg transition">✕</button>' +
    '</div></div>'
  ).join("");
}
...
$("chooseContactBtn").onclick = () => {
  ...
  openContactPicker((address, name) => {
    $("recipient").value = address;
    showToast("Selected: " + name, "info");
  });
};
...
$("saveScheduleBtn").onclick = async () => {
  if (!account) { showToast("Connect wallet first!", "error"); return; }
  const r = $("schedRecipient").value.trim(), a = $("schedAmount").value;
  const dt = $("schedDateTime").value, rep = $("schedRepeat").value, m = $("schedMemo").value.trim();
  if (!r || !a || !dt) { showToast("Fill in all fields", "error"); return; }
  const { error } = await supabase.from("scheduled_payments").insert({
    wallet: account.toLowerCase(), recipient: r, amount: parseFloat(a),
    memo: m, category: "Payment", scheduled_at: new Date(dt).toISOString(),
    repeat_type: rep, status: "pending",
  });
  if (error) { showToast("Error: " + error.message, "error"); return; }
...
function renderScheduled(items) {
  ...
  el.innerHTML = items.map(p => {
    const dt = new Date(p.scheduled_at), isDue = dt <= new Date();
    const name = getContactName(p.recipient);
    return (
      '<div class="flex items-center gap-2 px-3 py-2.5 rounded-xl border text-xs ' +
      (isDue ? 'border-violet-500/40 bg-violet-500/5' : 'bdr') + '">' +
      '<div class="text-base shrink-0">' + (isDue ? '⚡' : '⏰') + '</div>' +
      '<div class="flex-1 min-w-0 truncate">' +
      '<span class="font-medium">' + (name || p.recipient.slice(0,8)+"...") + ' </span>' +
      '<span class="t3">' + dt.toLocaleDateString("en-US",{month:"short",day:"numeric"}) + " " +
      dt.toLocaleTimeString("en-US",{hour:"2-digit",minute:"2-digit"}) + '</span>' +
      ...
      '<button onclick="deleteScheduled(' + p.id + ')" class="t3 hover:text-red-400 text-xs shrink-0 transition ml-1">✕</button>' +
      '</div>'
    );
  }).join("");
}
...
function scheduleNotification(p) {
  const ms = new Date(p.scheduled_at) - new Date();
  if (ms > 0 && ms < 3600000) setTimeout(() => { showToast('⏰ Payment due: ' + p.amount + ' USDC', "info"); loadScheduled(); }, ms);
}
...
$("miniEventType").addEventListener("change", () => {
  const t = $("miniEventType").value;
  $("miniPaymentFields").classList.toggle("hidden", t !== "payment");
  $("miniCustomFields").classList.toggle("hidden", t !== "custom");
});

$("miniPickContactBtn").onclick = () => {
  if (!allContacts.length) { showToast("No contacts yet", "info"); return; }
  openContactPicker((address, name) => {
    $("miniPayRecipient").value = address;
    showToast("Selected: " + name, "info");
  });
};
...
$("miniSaveEventBtn").onclick = async () => {
  ...
  if (type === "payment") {
    const recipient = $("miniPayRecipient").value.trim();
    const amount = $("miniPayAmount").value;
    const memo = $("miniEventTitle").value.trim() || "Scheduled transfer";
    ...
    const { error } = await supabase.from("scheduled_payments").insert({
      ...
      memo, category: "Payment", scheduled_at: new Date(dt).toISOString(),
      repeat_type: "once", status: "pending",
    });
    if (error) { showToast("Error: " + error.message, "error"); return; }
    await loadScheduled();
    showToast("Transfer scheduled ✓", "success");
  } else {
    const title = type === "custom"
      ? $("miniCustomLabel").value.trim()
      : $("miniEventTitle").value.trim();
  ...
  $("miniEventTitle").value = "";
  $("miniPayRecipient").value = "";
  $("miniPayAmount").value = "1";
  $("miniCustomLabel") && ($("miniCustomLabel").value = "");
...
function renderCalendar() {
  ...
  for (let day=1; day<=daysInMonth; day++) {
    ...
    html += '<div onclick="showMiniDayDetail('+day+','+year+','+month+')" class="py-1 rounded-lg text-center cursor-pointer flex flex-col items-center gap-0.5 hover:bg-zinc-800/40 transition ' +
      (isToday ? 'ring-1 ring-emerald-500/50 bg-emerald-500/10 text-emerald-400 font-semibold' : (hasTx||hasSched||hasEv ? 'font-medium' : 't3')) +
      ' text-xs">' + day + (dots.length ? '<div class="flex gap-0.5">'+dots.join("")+'</div>' : '<div class="h-1.5"></div>') + '</div>';
  }
...
window.showMiniDayDetail = (day, year, month) => {
  ...
  events.forEach(e => {
    html += '<div class="flex items-center justify-between text-xs py-1 px-2 rounded-lg" style="background:'+e.color+'18">' +
      ...
      '<button onclick="editMiniEvent('+e.id+',\''+e.title.replace(/'/g,"\\'")+'\')" class="t3 hover:text-blue-400 transition">✎</button>' +
      '<button onclick="deleteMiniEvent('+e.id+')" class="t3 hover:text-red-400 transition">✕</button>' +
      '</div></div>';
  });
  scheds.forEach(p => {
    html += '<div class="text-xs py-1 px-2 bg-violet-500/10 rounded-lg text-violet-300">💸 ' +
      (getContactName(p.recipient)||p.recipient.slice(0,8)+"...") + ' · ' + p.amount + ' USDC</div>';
  });
  txs.forEach(tx => {
    html += '<div class="flex justify-between text-xs py-1 px-2 rounded-lg" style="background:var(--card)">' +
      '<span class="t2">' + (getContactName(tx.recipient)||tx.recipient.slice(0,6)+"...") + '</span>' +
      '<span class="text-emerald-400">' + tx.amount + ' USDC</span></div>';
  });
...
window.editMiniEvent = async (id, oldTitle) => {
  const n = prompt("Edit event:", oldTitle);
  if (!n || n.trim() === oldTitle) return;
  await supabase.from("calendar_events").update({ title: n.trim() }).eq("id", id);
  await loadCalendarEvents();
  if (selectedMiniDay) showMiniDayDetail(selectedMiniDay.d, selectedMiniDay.y, selectedMiniDay.m);
...
$("modalPickContactBtn").onclick = () => {
  if (!allContacts.length) { showToast("No contacts yet", "info"); return; }
  openContactPicker((address, name) => {
    $("modalPayRecipient").value = address;
    showToast("Selected: " + name, "info");
  });
};
...
$("modalSaveEventBtn").onclick = async () => {
  ...
  if (type === "payment") {
    const r = $("modalPayRecipient").value.trim();
    const a = $("modalPayAmount").value;
    const memo = $("modalEventTitle").value.trim() || "Scheduled transfer";
...
window.deleteModalEvent = async id => {
  await supabase.from("calendar_events").delete().eq("id", id);
  await loadCalendarEvents(); renderModalCalendar();
  if (selectedModalDay) showModalDayDetail(selectedModalDay.d, selectedModalDay.y, selectedModalDay.m);
  showToast("Deleted", "info");
...
function renderModalCalendar() {
  ...
  for (let day=1; day<=daysInMonth; day++) {
    ...
    const evNames = hasEv ? hasEv.slice(0,1).map(e =>
      '<div class="text-xs leading-tight truncate px-0.5" style="color:'+e.color+'">' + (TYPE_ICONS[e.type]||"") + ' ' + e.title + '</div>'
    ).join("") : "";
    html += '<div onclick="showModalDayDetail('+day+','+year+','+month+')" class="py-1.5 rounded-xl cursor-pointer flex flex-col items-center hover:bg-zinc-800/60 transition min-h-[48px] justify-start pt-1.5 ' +
      (isToday ? 'ring-1 ring-emerald-500/50 bg-emerald-500/10 text-emerald-400 font-semibold' : (hasTx||hasSched||hasEv ? 'text-white' : 'text-zinc-500')) + '">' +
...
window.showModalDayDetail = (day, year, month) => {
  ...
  events.forEach(e => {
    html += '<div class="flex items-center justify-between p-2 rounded-xl text-xs" style="background:'+e.color+'18;border:1px solid '+e.color+'30">' +
      ...
      '<button onclick="editModalEvent('+e.id+',\''+e.title.replace(/'/g,"\\'")+'\')" class="text-zinc-500 hover:text-blue-400 transition">✎</button>' +
      '<button onclick="deleteModalEvent('+e.id+')" class="text-zinc-500 hover:text-red-400 transition">✕</button>' +
      '</div></div>';
  });
  scheds.forEach(p => {
    html += '<div class="p-2 bg-violet-500/10 border border-violet-500/20 rounded-xl text-xs text-violet-300">💸 ' +
      (getContactName(p.recipient)||p.recipient.slice(0,8)+"...") + ' · ' + p.amount + ' USDC</div>';
  });
  txs.forEach(tx => {
    html += '<div class="flex justify-between p-2 bg-zinc-800/60 rounded-xl text-xs">' +
