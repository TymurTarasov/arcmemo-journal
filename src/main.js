...
let activeMiniDayEl = null, activeModalDayEl = null;
...
// ─── MINI CALENDAR ───────────────────────────────────────────────────
$("prevMonth").onclick = () => { currentCalDate.setMonth(currentCalDate.getMonth()-1); renderCalendar(); $("miniDayDetail").classList.add("hidden"); selectedMiniDay = null; activeMiniDayEl = null; };
$("nextMonth").onclick = () => { currentCalDate.setMonth(currentCalDate.getMonth()+1); renderCalendar(); $("miniDayDetail").classList.add("hidden"); selectedMiniDay = null; activeMiniDayEl = null; };
...
$("miniPickContactBtn").onclick = () => {
  if (!allContacts.length) { showToast("No contacts yet", "info"); return; }
  openContactPicker((address, name) => { $("miniPayRecipient").value = address; showToast("Selected: " + name, "info"); });
};
...
function renderCalendar() {
  ...
  // Shift: Sunday=0 → position 6, Monday=1 → position 0
  const startOffset = (firstDay + 6) % 7;
  for (let i=0; i<startOffset; i++) html += "<div></div>";

  const today = new Date();
  const isSelectedDay = selectedMiniDay && selectedMiniDay.y===year && selectedMiniDay.m===month;

  for (let day=1; day<=daysInMonth; day++) {
    const isToday = today.getFullYear()===year && today.getMonth()===month && today.getDate()===day;
    const isSelected = isSelectedDay && selectedMiniDay.d === day;
    ...
    html += '<div onclick="showMiniDayDetail('+day+','+year+','+month+')" class="'+cls+'">'+
      day + (dots.length ? '<div class="flex gap-0.5">'+dots.join("")+'</div>' : '<div class="h-1.5"></div>') + '</div>';
  }
...
window.showMiniDayDetail = (day, year, month) => {
  ...
  txs.forEach(tx => {
    html += '<div class="flex justify-between text-xs py-1 px-2 rounded-lg" style="background:var(--card)">' +
      ...
      '<span class="t2 font-medium">' + (getContactName(tx.recipient)||tx.recipient.slice(0,6)+"...") + '</span>' +
      (tx.memo ? '<span class="t3 text-xs truncate">💬 ' + tx.memo + '</span>' : '') +
      '</div>' +
...
window.editMiniEvent = async (id, oldTitle, type) => {
  const n = prompt("Edit " + (type||"event") + ":", oldTitle);
...
$("modalPickContactBtn").onclick = () => {
  if (!allContacts.length) { showToast("No contacts yet", "info"); return; }
  openContactPicker((address, name) => { $("modalPayRecipient").value = address; showToast("Selected: " + name, "info"); });
};
...
window.editModalEvent = async (id, oldTitle, type) => {
  const n = prompt("Edit " + (type||"event") + ":", oldTitle);
  if (!n || n.trim() === oldTitle) return;
  await supabase.from("calendar_events").update({ title: n.trim() }).eq("id", id);
...
function renderModalCalendar() {
  ...
  for (let day=1; day<=daysInMonth; day++) {
    ...
    html += '<div onclick="showModalDayDetail('+day+','+year+','+month+')" class="'+cls+'">' +
...
window.showModalDayDetail = (day, year, month) => {
  ...
  txs.forEach(tx => {
    html += '<div class="p-2 rounded-xl text-xs" style="background:var(--card);border:1px solid var(--border)">' +
      ...
      '<span class="text-zinc-300 font-medium">' + (getContactName(tx.recipient)||tx.recipient.slice(0,8)+"...") + '</span>' +
